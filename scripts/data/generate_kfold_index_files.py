...
parser = ArgumentParser(
    description='Generates index files for train and validation, using k-fold cv split.',
...
def main():
    ...
    k = 1 if args.no_folds is None else args.no_folds
    for label_dict, label_file in zip(label_dicts, label_files):
        ...
        video_ids_in_use = np.asarray(video_ids_in_use)
        labels_in_use = np.asarray(labels_in_use)
        fold = 0
        for train_idx, valid_idx in skf.split(video_ids_in_use, labels_in_use):
            samples_train = video_ids_in_use[train_idx]
            y_train = labels_in_use[train_idx]
            samples_valid = video_ids_in_use[valid_idx]
            y_valid = labels_in_use[valid_idx]
            ...
            np.save(os.path.join(out_dir, 'valid_samples_' + file_name + file_ending + '.npy'), samples_valid)
            fold += 1
            # Print results
            print_res(y_train, y_valid)
