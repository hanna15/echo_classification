...
def get_metrics_probs(outputs, sm_prob_1s, targets, samples, binary=True, prefix=''):
    ...
    """
    if args.regression:  # The model outputs are also the actual predictions
        metrics = Metrics(targets, samples, preds=outputs, binary=binary, tb=True, regression=args.regression)
    else:
...
def run_batch(batch, model, criterion=None, binary=False):
    ...
    if criterion:
        if binary and not args.regression:
...
def train(model, train_loader, valid_loader, data_len, valid_len, tb_writer, run_name, optimizer, weights=None,
    ...
    for epoch in range(args.max_epochs):
        ...
        epoch_valid_samples = []
        epoch_valid_outs = []
        epoch_valid_prob_1s = None if args.regression else []
...
def main():
    ...
    train_dataset = EchoDataset(train_index_file_path, label_path, videos_dir=args.videos_dir,
                                cache_dir=args.cache_dir, transform=train_transforms,
                                scaling_factor=args.scaling_factor, procs=args.num_workers,
                                ...
                                segm_masks=args.segm_masks, temporal=args.temporal, clip_len=args.clip_len,
                                period=args.period, regression=args.regression)
    if args.weight_loss:
    ...
    valid_dataset = EchoDataset(valid_index_file_path, label_path, videos_dir=args.videos_dir, cache_dir=args.cache_dir,
                                ...
                                clip_len=args.clip_len, period=args.period, regression=args.regression)
    ...
    # Model & Optimizers
    num_classes = 1 if args.regression else len(train_dataset.labels)
    if args.temporal:
